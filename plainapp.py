...
# ==========================================
# 0. STYLING & RANDOM VIBRANT BUTTON ENGINE
# ==========================================
DARK_STYLESHEET = """
    QMainWindow, QDialog {
...
def apply_random_vibrant_style(button: QPushButton):
    vibrant_colors = [
        "#e74c3c", "#8e44ad", "#2980b9", "#27ae60", "#d35400",
        "#16a085", "#f39c12", "#c0392b", "#00b894", "#e84393",
...
class Repository:
    ...
    def add_task(self, task_type: TaskType, target: str, caption: str, file_path: str, scheduled_date: datetime, use_all: bool, randomize: bool, split: bool, profile_name: str = "All Profiles"):
        with SessionLocal() as db:
            t = Task(
                task_type=task_type,
...
class TimezoneDialog(QDialog):
    def __init__(self, parent=None):
        ...
        header.setStyleSheet("color: #00cec9;")
        ...
        apply_random_vibrant_style(btn_close)
...
class ProfileManagerTab(QWidget):
    def __init__(self, repo: Repository, refresh_callback=None):
        ...
        btn_open = QPushButton("🌐 Open Selected Profile (Login Chrome)")
        apply_random_vibrant_style(btn_open)
        ...
        apply_random_vibrant_style(btn_del)
        ...
        btn_browse = QPushButton("📁 Browse Profile Folder")
        apply_random_vibrant_style(btn_browse)
        ...
        apply_random_vibrant_style(btn_save_sett)
    ...
    def open_chrome_profile(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Chrome Launched", f"Opening Chrome Profile:\n{p_path}")
        except Exception as e:
...
# TAB 2: PHOTO TXT BATCH TAB (ENHANCED)
class PhotoTxtBatchTab(QWidget):
    def __init__(self, repo: Repository):
        ...
        # Profile Choice Dropdown (New Addition)
        ...
        self.txt_caption.setPlaceholderText("Enter message text or caption here...")

        # Photo File/Folder Selection Buttons
        self.txt_photo_path = QLineEdit()
        btn_photo_file = QPushButton("📸 Photo File")
        apply_random_vibrant_style(btn_photo_file)
        btn_photo_file.clicked.connect(self.browse_photo_file)

        btn_photo_dir = QPushButton("📁 Photo Folder")
        apply_random_vibrant_style(btn_photo_dir)
        ...
        # Date & Time Selector with AM / PM option
        self.dt_picker = QDateTimeEdit(QDateTime.currentDateTime())
        self.dt_picker.setDisplayFormat("yyyy-MM-dd hh:mm:ss AP") # AM/PM Format
        ...
        form.addRow("Schedule (AM/PM):", self.dt_picker)
        ...
        # Save Settings Button for Batch
        btn_save_batch = QPushButton("💾 Save Batch Configuration")
        apply_random_vibrant_style(btn_save_batch)
        btn_save_batch.clicked.connect(self.save_batch_config)
        layout.addWidget(btn_save_batch)

        btn_enqueue = QPushButton("🚀 Enqueue Scheduled Batch Task")
        apply_random_vibrant_style(btn_enqueue)
    ...
    def save_batch_config(self):
        time_str = self.dt_picker.dateTime().toString("yyyy-MM-dd hh:mm:ss AP")
        QMessageBox.information(
            self, "Batch Config Saved", 
            f"Batch Configuration Saved!\n\nProfile: {self.cmb_profile.currentText()}\nPhoto Path: {self.txt_photo_path.text()}\nText Path: {self.txt_text_path.text()}\nSchedule: {time_str}"
        )
    ...
    def enqueue_task(self):
        ...
        self.txt_target.clear(); self.txt_caption.clear(); self.txt_photo_path.clear(); self.txt_text_path.clear()
...
class TasksRunnerTab(QWidget):
    def __init__(self, repo: Repository, status_callback=None):
        ...
        self.btn_start = QPushButton("▶️ Start Parallel Runner Engine")
        apply_random_vibrant_style(self.btn_start)
        self.btn_start.clicked.connect(self.start_runner)

        self.btn_stop = QPushButton("⏹️ Stop Runner")
        apply_random_vibrant_style(self.btn_stop)
        self.btn_stop.clicked.connect(self.stop_runner)

        btn_refresh = QPushButton("🔄 Refresh Table")
        apply_random_vibrant_style(btn_refresh)
    ...
    def load_tasks(self):
        ...
        for row, t in enumerate(tasks):
            ...
            self.table.setItem(row, 3, QTableWidgetItem(t.scheduled_date.strftime("%Y-%m-%d %I:%M:%S %p")))
            ...
            self.table.setItem(row, 5, QTableWidgetItem(t.assigned_profile_name or "All Profiles"))
...
class PerformanceLogsTab(QWidget):
    def __init__(self, repo: Repository):
        ...
        apply_random_vibrant_style(btn_clear)
        btn_clear.clicked.connect(self.clear_logs)
        filter_layout.addWidget(btn_clear)

        btn_export = QPushButton("💾 Export Logs")
        apply_random_vibrant_style(btn_export)
        ...
        self.log_area.setStyleSheet("background-color: #0d0d0d; color: #00ff66; font-family: Consolas, Monospace; font-size: 13px;")
        layout.addWidget(self.log_area)

        # Live Timer for Log Auto-Refresh
        self.timer = QTimer(self)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # --- HEADER WITH STAR LOGO & TIMEZONE BUTTON ---
        header_layout = QHBoxLayout()

        star_header = QLabel("⭐ STAR AUTOMATION & SCHEDULER ⭐")
        star_header.setFont(QFont("Segoe UI", 16, QFont.Bold))
        star_header.setStyleSheet("color: #f39c12; padding: 5px;")
        header_layout.addWidget(star_header)

        header_layout.addStretch()

        btn_tz = QPushButton("🌐 World Timezones (10 Countries)")
        apply_random_vibrant_style(btn_tz)
